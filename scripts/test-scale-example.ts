import { getScaleFingering, getScaleNotes, validateFingeringNotes } from '../client/src/utils/scaleLibrary';
import { noteToValue, valueToNote, STANDARD_TUNING_NAMES } from '../client/src/utils/musicTheory';
...
function displayFingering(scaleName: string, rootNote: string, positionIndex: number, fingering: number[][]) {
  ...
  if (!validation.isValid && validation.invalidNotes.length > 0) {
    console.log(`Invalid notes found: ${validation.invalidNotes.slice(0, 5).join(', ')}`);
  }
}
