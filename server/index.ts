...
async function main() {
  ...
  if (!process.env.XAI_API_KEY) {
    ...
    throw new Error(
      "XAI_API_KEY environment variable is required. Please add it via Replit Secrets."
    );
  }
  ...
  if (typeof process.env.XAI_API_KEY === "string" && process.env.XAI_API_KEY.length < 10) {
    ...
    throw new Error(
      "XAI_API_KEY appears to be invalid. Please check your Replit Secrets configuration."
    );
  }
