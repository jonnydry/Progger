...
describe('API Routes', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(replitAuth.isAuthenticated).mockImplementation((req, res, next) => {
  ...
  describe('POST /api/generate-progression', () => {
    ...
    it('should accept valid progression request', async () => {
      ...
      vi.mocked(xaiService.generateChordProgression).mockResolvedValue(mockResult as any);
  ...
  describe('POST /api/analyze-custom-progression', () => {
    ...
    it('should accept valid custom progression', async () => {
      const mockResult = {
        progression: [
          { chordName: 'C', musicalFunction: 'Tonic', relationToKey: 'I' },
          { chordName: 'Am', musicalFunction: 'Submediant', relationToKey: 'vi' },
        ],
        scales: [{ name: 'C Major', rootNote: 'C' }],
      };

      vi.mocked(xaiService.analyzeCustomProgression).mockResolvedValue(mockResult as any);
  ...
  describe('GET /api/stash', () => {
    ...
    it('should return stash items for authenticated user', async () => {
      ...
      vi.mocked(storage.getUserStashItems).mockResolvedValue(mockItems as any);
  ...
  describe('POST /api/stash', () => {
    ...
    it('should create stash item with valid data', async () => {
      ...
      vi.mocked(storage.createStashItem).mockResolvedValue(mockItem as any);
