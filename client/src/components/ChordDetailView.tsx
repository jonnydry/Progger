...
function analyzeVoicing(
  ...
  const voicingNotes = Array.from(voicingNoteValues)
    .map((v) => valueToNote(v))
    .sort();

  const expectedNotes = new Set(
    getChordNotes(expectedChordName).map((note) => note.toLowerCase()),
  );
  const voicingNoteSet = new Set(
    voicingNotes.map((note) => note.toLowerCase()),
  );
...
const VoicingGroup: React.FC<{
  ...
  // Performance optimization: limit initial display to 8 voicings
  const INITIAL_LIMIT = 8;
  const displayedVoicings = showAll
    ? voicings
    : voicings.slice(0, INITIAL_LIMIT);
  const hasMore = voicings.length > INITIAL_LIMIT;

  return (
    <div className="mb-6">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full text-left text-md font-semibold text-text/80 mb-3 flex items-center gap-2 hover:text-text transition-colors group"
      >
        <svg
          className={`w-4 h-4 text-primary transition-transform duration-200 ${isExpanded ? "rotate-90" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        <>
          <div className="flex flex-row flex-wrap gap-6 overflow-x-auto">
            {displayedVoicings.map(({ voicing, chordName }, index) => {
              const displayedVariantName = displayChordName(
                chordName,
                musicalKey,
              );
              ...
              return (
                ...
                >
                  <VoicingDiagram
                    chordName={displayedVariantName}
                    voicing={voicing}
                  />
                  <div className="text-xs text-text/60 text-center max-w-[140px]">
                    <div className="font-semibold mb-1 text-text/80">
                      {voicing.position || "Standard"}
                    </div>
          ...
          {hasMore && !showAll && (
            <button
              ...
              className="mt-4 px-4 py-2 text-sm text-primary hover:text-accent font-semibold border-2 border-primary/30 hover:border-primary/60 transition-all rounded"
            >
              Show {voicings.length - INITIAL_LIMIT} More Voicings
            </button>
          )}
          ...
          {showAll && hasMore && (
            <button
              ...
              className="mt-4 px-4 py-2 text-sm text-text/70 hover:text-text font-semibold border-2 border-border hover:border-border/60 transition-all rounded"
            >
              Show Less
            </button>
          )}
...
export const ChordDetailView: React.FC<ChordDetailViewProps> = ({
  ...
  const [allVoicingsWithNames, setAllVoicingsWithNames] = React.useState<
    Array<{ voicing: ChordVoicing; chordName: string }>
  >([]);
  ...
  React.useEffect(() => {
    ...
    async function loadVoicings() {
      ...
      try {
        ...
        const variantPromises = candidateQualities.map(
          async (variantQuality) => {
            // Construct chord name
            let variantChordName = root;
            if (
              variantQuality === "maj7" ||
              variantQuality === "maj9" ||
              variantQuality === "maj13"
            ) {
              variantChordName += variantQuality;
            } else if (
              variantQuality === "min7" ||
              variantQuality === "min9" ||
              variantQuality === "min11" ||
              variantQuality === "min13"
            ) {
            ...
            const variantVoicings =
              await getChordVoicingsAsync(variantChordName);

            // Only include if we have real voicings (not all muted)
            if (
              variantVoicings.length > 0 &&
              !variantVoicings.every((v) => isMutedVoicing(v))
            ) {
              return variantVoicings.map((voicing) => ({
                voicing,
                chordName: variantChordName,
              }));
            }
            return [];
          },
  ...
  const voicingGroups = React.useMemo(() => {
    ...
    allVoicingsWithNames.forEach(({ voicing, chordName }) => {
      ...
      if (positionLower.includes("open")) {
        ...
        positionLower.includes("barre") ||
        position.includes("Barre")
      ) {
        groups["Barre"].push({ voicing, chordName });
      } else if (positionLower.includes("partial")) {
        groups["Partial"].push({ voicing, chordName });
      } else if (
        positionLower.includes("a-string root") ||
        positionLower.includes("a-string")
      ) {
    ...
    <div className="mt-8 max-w-6xl mx-auto w-full animate-slide-in">
      <div className="bg-surface rounded-lg p-6 shadow-2xl border border-border relative space-y-8">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-4xl font-bebas tracking-wider text-text/90">
              {displayedChordName}
            </h2>
        ...
        <div className="border-t border-border pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-background/50 rounded-lg p-4 border border-border/50">
              <h4 className="text-sm font-semibold text-text/70 uppercase tracking-wide mb-2">
                Formula
              </h4>
              <p className="text-lg font-mono text-text">
                {chordAnalysis.formula}
              </p>
            </div>
            <div className="bg-background/50 rounded-lg p-4 border border-border/50">
              <h4 className="text-sm font-semibold text-text/70 uppercase tracking-wide mb-2">
              ...
              <div className="flex flex-wrap gap-1">
                {chordAnalysis.intervals.map((interval, index) => (
                  <span
                    key={index}
                    className="text-sm font-mono bg-primary/10 text-primary px-2 py-0.5 rounded"
                  >
                    {interval}
                  </span>
                ))}
            ...
            <div className="bg-background/50 rounded-lg p-4 border border-border/50">
              ...
              <div className="flex flex-wrap gap-1">
                {chordAnalysis.notes.map((note, index) => (
                  <span
                    key={index}
                    className="text-sm font-mono bg-surface text-text px-2 py-0.5 rounded border border-border/30"
                  >
                    {note}
                  </span>
                ))}
            ...
            <div className="bg-background/50 rounded-lg p-4 border border-border/50">
              ...
              <div className="flex flex-wrap gap-1">
                {chordAnalysis.compatibleScales
                  .slice(0, 2)
                  .map((scale, index) => (
                    <span
                      key={index}
                ...
                {chordAnalysis.compatibleScales.length > 2 && (
                  <span className="text-xs text-text/50">
                    +{chordAnalysis.compatibleScales.length - 2} more
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-background/50 rounded-lg p-4 border border-border/50">
              ...
              <div className="text-sm text-text/80">
                <div className="mb-2">
                  <span className="font-semibold">Total Voicings:</span>{" "}
                  {allVoicingsWithNames.length}
                </div>
                <div className="mb-2">
                  <span className="font-semibold">Expected Notes:</span>{" "}
                  {chordAnalysis.notes.join(", ")}
                </div>
                <div className="text-xs text-text/60 mt-2">
                  Each voicing below shows which notes it contains. Missing or
                  extra notes are indicated.
                </div>
              </div>
            </div>
            <div className="bg-background/50 rounded-lg p-4 border border-border/50">
              ...
              <div className="text-sm text-text/80">
                ...
                <div className="text-xs text-text/60 mt-2">
                  Use barre voicings to transpose this chord shape to other
                  roots.
                </div>
              </div>
            </div>
        ...
        <div className="border-t border-border pt-6">
          ...
          {isLoadingVoicings ? (
            ...
            (() => {
              ...
              type VoicingWithName = {
                voicing: ChordVoicing;
                chordName: string;
              };
              const hasGroupedVoicings = (
                Object.values(voicingGroups) as VoicingWithName[][]
              ...
              if (!hasGroupedVoicings) {
                ...
                return (
                  <div className="flex flex-row flex-wrap gap-4 justify-start">
                    {allVoicingsWithNames.map(
                      ({ voicing, chordName }, index) => {
                        const displayedVariantName = displayChordName(
                          chordName,
                          musicalKey,
                        );
