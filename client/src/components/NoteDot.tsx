...
export const NoteDot: React.FC<NoteDotProps> = React.memo(
  ({ noteName, fret, isRoot, isHovered, onMouseEnter, onMouseLeave }) => {
    ...
    return (
      ...
      >
        <div
          className={`relative w-5 h-5 md:w-full md:h-full rounded-full flex items-center justify-center text-[8px] md:text-[10px] font-bold border-2 border-surface transition-all duration-200 ${isRoot ? rootClasses : noteClasses} ${isHovered ? "scale-110 z-20" : ""}`}
        >
          {noteName}
        </div>
