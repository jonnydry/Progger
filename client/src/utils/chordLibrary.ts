...
const CHORD_VOICINGS: Record<ChordKey, ChordVoicing[]> = {
  ...
  E_major_Aroot: [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 4, position: "A-string Root" }],
  E_minor_Aroot: [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 4, position: "A-string Root" }],

  // F root on A string (fret 8) - F is fret 8 on A string
  F_major_Aroot: [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 5, position: "A-string Root" }],
  F_minor_Aroot: [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 5, position: "A-string Root" }],

  // F# root on A string (fret 9) - F# is fret 9 on A string
  "F#_major_Aroot": [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 6, position: "A-string Root" }],
  "F#_minor_Aroot": [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 6, position: "A-string Root" }],

  // G root on A string (fret 10) - G is fret 10 on A string
  G_major_Aroot: [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 7, position: "A-string Root" }],
  G_minor_Aroot: [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 7, position: "A-string Root" }],

  // A# root on A string (fret 11) - A# is fret 11 on A string
  "A#_major_Aroot": [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 8, position: "A-string Root" }],
  "A#_minor_Aroot": [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 8, position: "A-string Root" }],

  // B root on A string (fret 12) - B is fret 12 on A string
  B_major_Aroot: [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 9, position: "A-string Root" }],
  B_minor_Aroot: [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 9, position: "A-string Root" }],

  // C# root on A string (fret 13) - C# is fret 13 on A string
  "C#_major_Aroot": [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 10, position: "A-string Root" }],
  "C#_minor_Aroot": [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 10, position: "A-string Root" }],

  // D# root on A string (fret 14) - D# is fret 14 on A string
  "D#_major_Aroot": [{ frets: [1, 4, 6, 6, 6, 4], firstFret: 11, position: "A-string Root" }],
  "D#_minor_Aroot": [{ frets: [1, 4, 6, 6, 5, 4], firstFret: 11, position: "A-string Root" }],
  ...
  A_power_Aroot: [{ frets: ["x", 0, 2, 2, "x", "x"], position: "A-string Root" }],

  // E root on A string (alternative voicing)
  E_power_Aroot: [{ frets: ["x", 1, 3, 3, "x", "x"], firstFret: 7, position: "A-string Root" }],

  // D root on A string (partial chord)
  D_power_Aroot: [{ frets: ["x", "x", 7, 7, "x", "x"], firstFret: 10, position: "A-string Root" }],
...
export function normalizeRoot(root: string): string {
  ...
  console.warn(
    `Root note "${root}" normalized to "${firstPossibility}" (no chords found for this or enharmonic equivalents)`
  );
...
export function findClosestChordVoicings(chordName: string): ChordVoicing[] {
  ...
  for (const [key, voicings] of Object.entries(CHORD_VOICINGS)) {
    ...
    if (similarity > 10) {
      // Only consider reasonably similar chords
  ...
  for (const candidate of candidates) {
    ...
    if (chordRoot === root && chordQuality === quality) {
      ...
      if (validatedVoicings.length > 0) {
        console.info(
          `✅ Validated fallback: "${chordName}" (${targetKey}) → "${candidate.key}" (${validatedVoicings.length} valid voicings)`
        );
    ...
    if (chordQuality === quality && chordRoot !== root) {
      ...
      if (validatedVoicings.length > 0) {
        console.info(
          `✅ Transposed fallback: "${chordName}" (${targetKey}) → "${candidate.key}" transposed from ${chordRoot} to ${root}`
        );
        return adjustVoicingsForSlashBass(validatedVoicings, slashBass);
      }
    }
    ...
    if (chordRoot === root && chordQuality !== quality) {
      ...
      if (validatedVoicings.length > 0) {
        console.info(
          `✅ Validated fallback (different quality): "${chordName}" (${targetKey}) → "${candidate.key}"`
        );
        return adjustVoicingsForSlashBass(validatedVoicings, slashBass);
      }
    }
  ...
  const validatedGeneric = genericVoicings.filter((v) => validateVoicingMatchesChord(v, chordName));
...
export function getChordVoicings(chordName: string): ChordVoicing[] {
  ...
  console.warn(
    `No voicings found for "${chordName}". Please migrate to getChordVoicingsAsync() for transposition fallback.`
  );
  return [];
}
...
function adjustVoicingsForSlashBass(voicings: ChordVoicing[], slashBass?: string): ChordVoicing[] {
  ...
  return voicings.map((voicing) => {
    const noteValues = voicing.frets.map((fret, index) => {
      ...
      const absoluteFret =
        voicing.firstFret && voicing.firstFret > 1 ? voicing.firstFret + fret - 1 : fret;
      return (STANDARD_TUNING_VALUES[index] + absoluteFret) % 12;
    });

    const lowestSoundingIndex = noteValues.findIndex(
      (value, idx) => value !== null && voicing.frets[idx] !== "x"
    );
    ...
    const updatedFrets = voicing.frets.map((fret, idx) => (idx < targetIndex ? "x" : fret));
