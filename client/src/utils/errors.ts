...
export class ScaleNotSupportedError extends MusicTheoryError {
  ...
  async recover() {
    ...
    return (
      scaleNames.find(
        (scale) => scale.toLowerCase().includes("major") || scale.toLowerCase().includes("minor")
      ) || "major"
    );
  }
}
...
export class ScaleFingeringError extends MusicTheoryError {
  constructor(
    ...
    super(
      `Cannot generate fingering for scale "${scaleName}" (${rootNote}): ${fingeringError}`,
      "scale_fingering"
    );
...
export class InvalidAPIResponseError extends MusicTheoryError {
  constructor(
    ...
    super(`Invalid API response format. Expected: ${expectedFormat}`, "api_validation");
...
export class ProcessingPipelineError extends MusicTheoryError {
  ...
  async recover(successfulResults: unknown[]) {
    console.warn(
      `🔄 Processing pipeline error - returning ${successfulResults.length} successful results`
    );
...
export function isRecoverableError(
  error: unknown
): error is MusicTheoryError & { recover: () => Promise<unknown> } {
  return (
    error instanceof MusicTheoryError &&
    "recover" in error &&
    typeof (error as Record<string, unknown>).recover === "function"
  );
}
...
function deriveFallbackParameters(cacheKey?: string): {
  key: string;
  mode: string;
  numChords: number;
} {
...
async function buildLocalFallbackProgression(
  key: string,
  numChords: number,
  mode: string = "ionian"
) {
  ...
  const voicingsPromises = chordNames
    .slice(0, numChords)
    .map((chordName) => getChordVoicingsAsync(chordName));
