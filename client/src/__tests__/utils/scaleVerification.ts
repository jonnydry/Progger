...
function verifyCMajorPositions() {
  ...
  for (let pos = 0; pos < 5; pos++) {
    ...
    console.log(
      `  ✓ Low E root fret: ${minFret}${matchesExpectedOrigin ? " ✓" : " ❌ (expected " + expectedOrigin + ")"}`
    );
    console.log("");
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(
    "• Modal scales should start at characteristic fret positions (10th for D, 12th for E, etc.)"
  );
...
export { verifyCMajorPositions, verifyMinorPentatonicBoxes, verifySingleScalePattern };
